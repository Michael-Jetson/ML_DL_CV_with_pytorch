...
一个单词是可以使用一个很大的实数向量来表示的，同时对于人类来说，语言是容易理解的，但是对于计算机来说，这是很难理解和表达的，或者这与语言系统是由人类构建的事实有关
...
那么问题就来了，模型是如何理解语言的含义的呢，这就涉及我们如何表示一个词的含义了，根据韦氏词典的定义，一个词的**含义（meaning）**是一个词所代表的想法，或者说一个人想通过使用单词符号表达的想法，或者说韦氏词典对单词含义的定义集中在想法这个词上，实际上这很接近语言学家思考意义的最常见方式，他们认为词义是作为能连接语言符号（或者说一个词所表示的符号）与其所表示的具体观念或事物之间的配对，比如说“椅子”这个词表示的就是椅子的集合，这也被称为语义，这种术语也被应用在编程语义的语义中，当然这个模型的实施性不是很深入，比如说很难从“椅子”意味着世界上所有的椅子的集合转换为可以使用计算机操纵的意义
...
这是一种传统的NLP方法，关键词可以理解为从大量文本中筛选出来的非常有代表性的少数的词语，比如说给出一篇文章的关键词我们就可以理解这个文章在表达什么

然后就是停用词，在文章中会有大量的几乎没有实际语义的语气词和符号，为了删除这些词与符号也就是停用词，很多机构提出了停用词表，也就是可以认为表中存在的词是可以去除的，比较出名的停用词表要哈工大、百度等提出的停用词表

分词技术是为了把句子或者文章分成一个个可以表达完整意思的单词，因为中文的最小单位是汉字，而一般两个及以上的汉字才可以构成一个单词，所以需要对中文进行准确的分词，比如说“这个故事有很高的教育价值”这句话，就可以分为“这个”、“故事”、“有”、“很高的”、“教育”、“价值”等词，每个词都可以完整的表达一种意思

对于一个文章的处理就是，先使用分词技术分成若干的词语，然后使用停用词表筛除那些在停用词表中的词（这一步可以使用for循环，如果一个单词不在停用词表中就可以添加到词汇列表中）
